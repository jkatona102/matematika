...
Uobičajena oznaka funkcijskih vrijednosti je $a(n)=a_n$, a oznaka niz $(a_n)_{n \in \mathbb{N}}$, $(a_n)_n$ ili samo $(a_n)$.
...
Tada $a$ zovemo **granična vrijednost** ili **limes** niza $(a_n)_n$ i pišemo $a = lim_{n -> \infty} a_n$.
...
1. Niz $(a_n \pm b_n)_n$ je konvergentan i vrijedi $\lim_{n \to \infty}(a_n \pm b_n)_n = \lim_{n \to \infty} a_n \pm \lim_{n \to \infty} b_n$
2. Niz $(a_n \cdot b_n)_n$ je konvergentan i $\lim_{n \to \infty} (a_n \cdot b_n)_n = \lim_{n \to \infty} a_n \cdot \lim_{n \to \infty} b_n$
3. Ako je $\forall n \in \mathbb{N}, b_n \neq 0$ i $\lim_{n \to \infty} b_n \neq 0$, onda je niz $\left (\frac{a_n}{b_n} \right)_n$ konvergentan i vrijedi $\displaystyle \lim_{n \to \infty} \left (\frac{a_n}{b_n} \right)_n = \frac{\lim_{n \to \infty} a_n}{\lim_{n \to \infty} b_n}$ 
4. Niz $(|a_n|)_n$ je konvergentan i $\lim_{n \to \infty} |a_n| = |\lim_{n \to \infty} a_n|$
:::

:::{.korolar}
Neka su $\lim_{n \to \infty} a_n = a$ i $\lim_{n \to \infty} b_n = b$. Tada je za sve $\lambda, \mi \in \mathbb{R}$ niz $(\lambda a_n + \mi b_n)_n$ konvergent i vrijedi $\lim_{n \to \infty} (\lambda a_n + \mi b_n) = \lambda a + \mi b$.
:::
...
:::{.def}
Neka je $I \subseteq \mathbb{R}$ otvoreni interval i $c \in I$. Za funkciju $f \colon I \backslash \{c\} \to \mathbb{R}$ kažemo da ima limes u točki $c$ jedank $L$ ako **za svaki** niz $(c_n)_n$ u $I \backslash \{c\}$ vrijedi 
$$ 
\lim_{n \to \infty} c_n = c \longrightarrow \lim_{n \to \infty} f(c_n) = L
$$  

Tada pišemo $\lim_{x \to c} f(x) = L$.
:::

Iz definicije je vidljivo da je približavanje broja $x$ prema $c$ ekvivalentno približavanju svih nizova koji konvergiraju k $c$. Dakle, u definiciji je potrebno promatrati sve takve nizove, a ne samo jedan niz ili njih konačno mnogo. Tada se slike tih nizova po funkciji $f$ moraju približavati istom broju $L$. S druge strane, nizovi koji konvergiraju k $c$ imaju točke i lijevo i desno od $c$, pa je prirodno pretpostaviti da je točka $c$ iz nekog otvorenog intervala koji je sadržan u domeni funkcije $f$.

Pojam limesa funkcije moguće je definirati i bez nizova i to je sadržaj Cauchyjeve definicije limesa:

:::{.teorem}
Neka je $I \subseteq \mathbb{R}$ otvoreni interval, $c \in I$ i $f \colon I \backslash \{c\} \to \mathbb{R}$. Limes funkciej $f$ u točki $c$ postoji i $\lim_{x \to c} f(x) = L$ ako i samo ako vrijedi:
$$
...
1. Funkcija $f \pm g$ ima limes u $c$ i vrijedi $\lim_{x \to c} (f(x) \pm g(x)) = \lim_{x \to c} f(x) \pm \lim_{x \to c} g(x)$
2. Za svaki $\lambda \in \mathbb{R}$ funkcija $\lambda f$ ima limes u $c$ i vrijedi $\lim_{x \to c} \lamda f(x) = \lamda \lim_{x \to c} f(x)$
3. Funkcije $fg$ ima limes u $c$ i  vrijedi $\lim_{x \to c} (f(x)g(x)) = \lim_{x \to c} f(x) \lim_{x \to c} g(x)$
3. Ako je $ g(x) \neq 0, \forall x \in I \backslash \{c\}$ i $\lim_{x \to c} g(x) \neq 0$, onda je funkcija $\left (\frac{f}{g} \right)_n$ ima limes u $c$ i vrijedi $\displaystyle \lim_{x \to c} \left (\frac{f(x)}{g(x)} \right)_n = \frac{\lim_{x \to c} f(x)}{\lim_{x \to c} g}$ 
4. Funkcija $|f|$ ima limes u $c$ i $\lim_{x \to c} |f(x)| = |\lim_{x \to c} f(x)|$
:::
